...
class lowerValueTrigger extends BaseController
{
  ...
  {/*
    ...
    {
        if($check_existingTrigger[0]->status == "disabled"){
        }
        else
        {
          //update existing row
          DB::connection("mysql")->table("DataTriggers")->where(['Device_ID' => $deviceID, 'Data_Path' => $path])->update(['status' => 'disabled']);
        ...
        //create new row
        DB::connection("mysql")->table("DataTriggers")->insert(['Device_ID' => $deviceID, 'Data_Path' => $path, 'status' => 'disabled']);
  ...
  public function sendMail($data){
    $text = 'Buonasera da '.$data->DeviceName."!";
    $email = $data->email;
    try {
